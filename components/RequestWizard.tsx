...
export const RequestWizard: React.FC<Props> = ({ user, onCancel, onSuccess }) => {
  ...
  const handleCreate = async () => {
    ...
    // 2. 7-DAY LOCK CHECK
    const [orderYear, orderMonth, orderDay] = formData.orderDate.split('-').map(Number);
    const [actionYear, actionMonth, actionDay] = formData.date.split('-').map(Number);

    const orderDate = new Date(orderYear, orderMonth - 1, orderDay, 0, 0, 0);
    const actionDate = new Date(actionYear, actionMonth - 1, actionDay, 0, 0, 0);

    const minDate = new Date(orderDate.getTime());
    minDate.setDate(minDate.getDate() + 7);

    if (actionDate.getTime() < minDate.getTime()) {
        toast.error("A Data da Ação deve ter no mínimo 7 dias de antecedência do Pedido.");
        return;
    }
