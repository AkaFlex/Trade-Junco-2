...
import { PlusCircle, ShoppingBag, DollarSign, ShieldAlert, ArrowRight } from 'lucide-react';

interface Props {
  user: UserProfile | null;
  onNavigate: (flow: 'request' | 'sellout' | 'reimbursement' | 'admin') => void;
}
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto w-full animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 mt-12">
        ...
        >
          ...
          <h2 className="text-xl font-bold text-gray-800 mb-3 relative z-10">Solicitar Reembolso</h2>
          <p className="text-gray-500 text-sm mb-6 relative z-10 flex-1">
            Envie fotos e compravantes para receber o pagamento de uma ação.
          </p>
          <span className="text-purple-600 font-bold text-sm flex items-center gap-2 relative z-10">
            ENVIAR DADOS <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform"/>
          </span>
        </button>
      </div>
